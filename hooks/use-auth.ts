...
export function useAuth(): UseAuthReturn {
  ...
  const updateAccountBalance = useCallback((accountId: string, newBalance: number | string) => {
    ...
    setAccounts(prevAccounts => {
      const targetId = accountId ? accountId.trim().toUpperCase() : '';
      let matched = false;
      const next = prevAccounts.map(acc => {
        if (acc.account_id && acc.account_id.trim().toUpperCase() === targetId) {
          matched = true;
          return { ...acc, balance: String(numeric) };
        }
        return acc;
      });

      // If specific ID was not matched but we have accounts, update the active or single account
      if (!matched && prevAccounts.length > 0) {
        const fallback = prevAccounts.map((acc, idx) => {
          if (idx === 0 || acc.account_id === activeAccountIdRef.current) {
            return { ...acc, balance: String(numeric) };
          }
          return acc;
        });
        storeDerivAccounts(fallback);
        return fallback;
      }
