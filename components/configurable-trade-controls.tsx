...
export function ConfigurableTradeControls(props: ConfigurableTradeControlsProps) {
  ...
  const handleBuyClick = () => {
    if (!isAuthenticated || props.authState !== 'authenticated') {
  ...
  const renderBuy = () => {
    const isAuthed = isAuthenticated && props.authState === 'authenticated';
    const disabled = isAuthed && (!isConnected || !proposal || isBuying);
      ...
      b: () => (
        ...
        >
          <span className="flex w-full items-center justify-between px-1">
            <span>{isBuying ? 'Purchasing...' : !isAuthed ? 'Log In to Trade' : 'Buy'}</span>
            {payout && isAuthed && (
        ...
        >
          <span className="flex flex-col items-center leading-tight gap-0.5">
            <span className="flex items-center gap-1.5 font-semibold">
              ...
              {isBuying ? 'Purchasing...' : !isAuthed ? 'Log In to Trade' : 'Buy'}
            </span>
            {payout && isAuthed && <span className="text-xs font-normal opacity-90">Payout {payout} USD</span>}
          </span>
        </Button>
      ),
    };
